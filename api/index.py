...
# Load .env for local development via `vercel dev`.
# On Vercel production, OPENROUTER_API_KEY is set via the dashboard.
...
if _key:
    ...
    print(f"[startup] WARNING: OPENROUTER_API_KEY not set. Add it in the Vercel dashboard.")

app = FastAPI()

# Allow all origins so the frontend (same Vercel deployment) and local dev both work.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Each cold start resets the counter. For strict enforcement, replace _usage
# with a persistent store (e.g. Vercel KV, Upstash Redis).
FREE_LIMIT = 3
...
def generate(
    ...
    if not api_key:
        raise HTTPException(
            status_code=500,
            detail="OPENROUTER_API_KEY is not set. Add it in the Vercel dashboard under Environment Variables.",
        )
